...
definition seq_s :: "D \<Rightarrow> D \<Rightarrow> D" ("_;;s/ _" [60, 61] 60)
  where "seq_s s t e = SetToPd (\<Union>{PdToSet (t x) | x. E x \<in> PdToSet (s e)} 
                               \<union> {x | x. x \<in> PdToSet (s e) \<inter> {Div, Err}})"

definition lchoice_s :: "D \<Rightarrow> D \<Rightarrow> D" ("_<+s/_" [60, 61] 60)
  where "lchoice_s s t e = SetToPd ((PdToSet (s e) - {Err}) 
                                   \<union> {x | x. x \<in> PdToSet (t e) \<and> Err \<in> PdToSet (s e)})"

definition choice_s ::  "D \<Rightarrow> D \<Rightarrow> D" ("_><s/_" [60, 61] 60)
  where "choice_s s t e = SetToPd ({E x | x. E x \<in> PdToSet (s e)}
...
definition one_s :: "D \<Rightarrow> D" 
  where "one_s s e = SetToPd ({E (Node l x e2) | l x e1 e2. e = Node l e1 e2 \<and> E x \<in> PdToSet (s e1)}
...
definition some_s :: "D \<Rightarrow> D" 
  where "some_s s e = SetToPd ({E (Node l x1 x2) | l x1 x2 e1 e2. e = Node l e1 e2 \<and> E x1 \<in> PdToSet (s e1) \<and> E x2 \<in> PdToSet (s e2)}
...
theorem exec_topDown : "exec (topDown s) \<xi> = exec (mu 0. (s <+ one \<lparr>0\<rparr>)) \<xi>"
  using TopDown_def by fastforce
